...
def ConvLayer(in_channels, out_channels, kernel_size=3, stride=1, 
    upsample=None, instance_norm=True, relu=True, trainable=True):
    layers = []
    if upsample:
...
class TransformNet(nn.Module):
    ...
    def get_param_dict(self):
        """找出该网络所有 MyConv2D 层，计算它们需要的权值数量（繁）"""
...
if __name__ == '__main__':
    tnet = TransformNet(32)
    
    print(tnet.get_param_dict2() == tnet.get_param_dict())
